...
class CommandClassGenerator {
	...
	private _enums: Map<string, spec.EnumValues> = new Map();
	private _enumsCanonical: Map<string, string> = new Map();
	private _incompleteCommands: Set<number> = new Set();
	...
	private _generateEnums(): string[] {
		...
		for (const [name, values] of this._enums) {
			...
			for (const indexStr of Object.keys(values)) {
	...
	private _generateCommandClass(className: string): Lines {
		...
		if (this._class.status !== spec.ObsolescenceStatus.Active) {
			contents.push(
				"// This (version of the) command class is " +
		...
		contents.push(
			...indent([
				`public static readonly commandClass: number = CommandClasses.${
					this._class.name
				}; // 0x${this._class.commandClass
					.toString(16)
					.padStart(2, "0")} (${this._class.commandClass})`,
				`public static readonly version: number = ${this._class.version};`,
			])
		);
		if (commandMask) {
			contents.push(
				...indent([
					`public static readonly commandMask: number = 0x${commandMask.toString(
						16
					)};`,
				])
			);
	...
	private _generateCommands(className: string): string[] {
		...
		for (const command of this._class.commands) {
			...
			contents.push(
				...indent([
					`public static readonly CommandClass: typeof ${className} = ${className};`,
					`public static readonly command: number = 0x${command.command
						.toString(16)
						.padStart(2, "0")}; // ${command.command}`,
					`public static readonly definition: CommandDefinition = convertFromJsonCommand(${jsonifyCommandDefinition(
						command
