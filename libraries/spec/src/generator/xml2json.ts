			...
			case ParamType.MULTI_ARRAY:
				{
					...
					const descloc = (
						elems.find(
							(elem) =>
								(elem as MultiArrayParamDescLoc).paramdescloc
					...
					const bitflags = elems.filter(
						(elem) => (elem as MultiArrayParamBitFlags).bitflag
...
function generateCommand(
	...
	const command: jsonSpec.JsonCommandDefinition = {
		command: cmd.key,
 ...
 */
function collapseEncryptedPayload(command: spec.CommandDefinition): void {
...
function collapseCommandEncapsulation(command: spec.CommandDefinition): void {
